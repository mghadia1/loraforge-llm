...
UNRECORDED = "not recorded by this run (it predates the parameter audit)"


def _percent(report: dict[str, Any]) -> str:
    """Report the audited parameter count, or say plainly that it was not measured.

    Returning a zero here once put "Trainable parameters: **0**" on a published
    card. A missing measurement must read as missing, never as a value.
    """
