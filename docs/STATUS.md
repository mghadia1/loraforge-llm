...
`Params4bit.numel()` undercounted the full denominator. The code and tests now
reject the packed denominator.
...
`outputs/training-report.json`. Until then, the setup-only numerator and its
derived percentage are both omitted from public result claims.
